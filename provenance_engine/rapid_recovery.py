...
class RapidOutputRecoveryProcessor:
    ...
    def process_case(self, pdf_path: Path) -> PredictionRow:
        ...
        final_row = apply_visible_finding_decision(final_row, pdf_path)
        final_row = apply_damage_weak_review(final_row, pdf_path)
        final_row = apply_approval_safety_demotion(
            final_row, pdf_path, candidates=tuple(primary_candidates)
        )
        final_row = apply_denial_to_review_softening(final_row)
        final_row = apply_visible_finding_decision(final_row, pdf_path)
        # Hard gate: never leave APPROVED on a transit visa.
        if (
            final_row.visa_class == "TRANSIT-7"
            and final_row.adjudication == "APPROVED"
        ):
            payload = final_row.to_dict()
            payload["adjudication"] = "DENIED"
        ...
        # Calibration-only blend. Never changes labels.
        final_row = apply_confidence_blend(final_row)
        return final_row
